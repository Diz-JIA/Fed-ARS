...
config = {
    ...
    "LOAD_SAVED_MODEL": False,  # 设置为True则跳过训练，直接加载模型进行分析
    "MODEL_SAVE_PATH": "./saved_model_pi/best_model.pth",
}
...
def internal_consistency_analysis(global_model_state_dict, client_update, device):
    ...
    perturbation = torch.randn(1, 3, 32, 32).to(device) * config["PERTURBATION_STRENGTH"]
...
if __name__ == "__main__":
    ...
    if config["LOAD_SAVED_MODEL"]:
        ...
        try:
            for round_num in range(config["FL_ROUNDS"]):
                ...
                print(
                    f"Epoch{round_num + 1}/{config['FL_ROUNDS']} | 当前LR: {current_lr:.6f} | Avg_Train_Loss: {avg_train_loss:.4f} | Test Loss: {test_loss:.4f} | Global_Test_Accuracy: {test_accuracy * 100:.2f}%")
    ...
    malicious_update, _ = malicious_client.train(copy.deepcopy(final_global_model_state_dict), learning_rate=current_lr)
    ...
    malicious_score = internal_consistency_analysis(copy.deepcopy(final_global_model_state_dict), malicious_update, DEVICE)
    benign_score = internal_consistency_analysis(copy.deepcopy(final_global_model_state_dict), benign_update, DEVICE)

    # 5. 打印结果
    print("\n--- 实验结果 ---")
    print(f"良性客户端更新的一致性得分: {benign_score:.6f}")
    print(f"恶意客户端更新的一致性得分: {malicious_score:.6f}")
    print(f"恶意客户端得分/良性客户端得分: {malicious_score/benign_score:.6f}")
    if malicious_score > benign_score * 2:
        print("\n[结论]: 实验成功！")
    else:
        print("\n[结论]: 实验结果不显著。")
