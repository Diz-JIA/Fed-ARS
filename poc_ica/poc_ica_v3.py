...
# @Author  : dizjia
# @File    : poc_ica_v3.py
# @Description: “内部一致性分析”概念验证实验的第三版。
#               核心修改：使用一张随机真实测试集图像探测➡️使用一组随机真实测试集图像探测。
...
config = {
    ...
    "MODEL_SAVE_PATH": "./saved_model_pi/best_model_20.pth",
}
...
if __name__ == "__main__":
    ...
    # 4. 关键验证步骤
    print(f"\n--- 关键验证：使用 {config['NUM_PROBES']} 个探测器进行集成分析 ---")
    ...
    # 使用当前时间作为种子，以确保每次抽样的随机性
    # 这样可以覆盖掉脚本开头设置的全局固定种子 a
    ...
    # 使用时间种子，确保每次“集成探测”时，选取的这一组图片都是随机的
    ...
    for i in range(config["NUM_PROBES"]):
        ...
        #print(f"    (探测器 {i + 1}/{config['NUM_PROBES']} 使用图片索引: {random_image_index})")

        # 2. 对良性更新进行分析
        b_score = internal_consistency_analysis(copy.deepcopy(final_global_model_state_dict), benign_update, DEVICE,
                                                probe_image_sample)
        benign_scores.append(b_score)

        # 3. 对恶意更新进行分析
        m_score = internal_consistency_analysis(copy.deepcopy(final_global_model_state_dict), malicious_update, DEVICE,
                                                probe_image_sample)
        malicious_scores.append(m_score)

    # 4. 聚合得分 (这里我们可以计算平均值，也可以计算最大值等)
    final_benign_score = np.max(benign_scores)
    final_malicious_score = np.max(malicious_scores)

    # --- 5. 打印结果 ---
    print("\n--- 实验结果 (集成探测) ---")
    print(f"良性客户端更新的一致性得分: {final_benign_score:.6f}")
    print(f"恶意客户端更新的一致性得分: {final_malicious_score:.6f}")

    if final_malicious_score > 0:  # 避免除以零
        ratio = final_malicious_score / final_benign_score
        print(f"恶意/良性比率: {ratio:.6f}")

    # (结论判断逻辑)
    if final_malicious_score > final_benign_score * 1.5:  # 我们可以设定一个更严格的阈值，例如1.5倍
        print("\n[结论]: 实验成功！恶意更新的平均得分显著高于良性更新。")
    else:
        print("\n[结论]: 实验结果不显著。")
