...
- 2025/09/19, v1.1, dizjia:
  - 之前设计的声誉逻辑（ic_ddw_wandb.py保留此设计）：如果客户端本轮可疑，声誉 +1；如果客户端本轮清白，声誉立刻清零
  - 修改后：如果客户端可疑，声誉分 +1；声誉分达到上限后永久降权。
"""
...
config = {
    ...
    # 实验日志文件路径
    "LOG_FILE_PATH": "./ic_ddw_simulation_log_scene3_new.csv",
...
class Server:
    def __init__(self, all_clients, test_loader, config):
        ...
        # [新增] 创建一个集合，用于存放已被“定罪”并需要永久降权的客户端ID
        self.permanently_downweighted_clients = set()
    ...
    def run_simulation(self):
        """运行完整的联邦学习模拟过程 (v3.2 - 永久声誉的动态降权软防御)"""
        try:
                ...
                if self.config["DEFENSE_ENABLED"] and t + 1 >= self.config["DEFENSE_START_ROUND"]:
                    ...
                    for client_id in suspicious_ids:
                        ...
                        if self.reputation_scores[client_id] >= self.config["REPUTATION_THRESHOLD"]:
                            if client_id not in self.permanently_downweighted_clients:
                                print(f"    [定罪模块] 客户端 {client_id} 声誉分达到上限，已被永久标记为不可信。")
                                self.permanently_downweighted_clients.add(client_id)

                    # print(f"    [声誉模块] 当前所有客户端声誉分: {self.reputation_scores}")
                    # --- 5. 模型加权聚合 (基于“罪犯名单”) ---
                    weights = {cid: self.config[
                        "SUSPICIOUS_CLIENT_WEIGHT"] if cid in self.permanently_downweighted_clients else 1.0
...
if __name__ == "__main__":
    ...
    if 3 in scenarios_to_run:
        ...
        params_log_3.update({
            ...
            "SIMILARITY_THRESHOLD": config.get("SIMILARITY_THRESHOLD", "N/A"),  # 增加对新参数的记录
            "REPUTATION_DECAY_FACTOR": config.get("REPUTATION_DECAY_FACTOR", "N/A")  # 增加对新参数的记录
        })
        actual_rounds_3 = len(history_with_defense['accuracy'])
        # [核心修改] 从server对象中获取最终被永久降权的客户端数量
