# -*- coding: utf-8 -*-
"""
@File    : ic_ddw_wandb.py
@Time    : 2025/9/19
@Author  : dizjia
@Description: IDA+余弦相似度+动态降权（Dynamic Down-weighting）
            硬防御➡软防御
            我们不再对客户端做“非黑即白”的生死审判。而是基于我们（不完全可靠的）侦测结果，对它们的“话语权”进行动态调整。
            对于那些我们高度信任的客户端，我们给予 100% 的话语权。
            对于那些我们有所怀疑的客户端，我们不是杀死它，而是暂时降低它的话语权（例如，只给它20%的权重）。

@History :
- 2025/09/19, v1.1, dizjia:
  - 增加历史声誉机制，只有被标记为“可疑”达到三次才进行降权
"""
...
class Server:
    ...
    def run_simulation(self):
        """运行完整的联邦学习模拟过程 (v3.0 - 基于声誉的动态降权软防御)"""
        try:
            for t in range(self.config["FL_ROUNDS"]):
                ...
                if self.config["DEFENSE_ENABLED"] and t + 1 >= self.config["DEFENSE_START_ROUND"]:
                    ...
                    if suspicious_ids:
                        print(f"    [侦测模块] 检测到可疑客户端: {suspicious_ids}")

                    # [Wandb] 3. 记录本轮防御的详情
                    wandb.log({
                        "round": t,
                        "suspicious_count": len(suspicious_ids),
                        "malicious_in_suspicious": len(
                            [cid for cid in suspicious_ids if cid < self.config["MALICIOUS_CLIENTS"]]),
                        "benign_in_suspicious": len(
                            [cid for cid in suspicious_ids if cid >= self.config["MALICIOUS_CLIENTS"]])
                    })

                    # --- 4. [核心修改] 更新声誉分，并决定谁需要被“降权” ---
                    clients_to_downweight = []
                    for client in selected_clients:
                        client_id = client.client_id
                        if client_id in suspicious_ids:
                            # 如果被怀疑，声誉分+1
                            self.reputation_scores[client_id] += 1
                        else:
                            # 如果本轮清白，声誉分立刻清零（声誉恢复机制）
                            self.reputation_scores[client_id] = 0

                        # 只有当声誉分累计达到阈值时，才将其加入“降权”名单
                        if self.reputation_scores[client_id] >= self.config["REPUTATION_THRESHOLD"]:
                            clients_to_downweight.append(client_id)

                    if clients_to_downweight:
                        print(f"    [降权模块] 声誉分达到上限，将对以下客户端降权: {clients_to_downweight}")

                    # --- 5. 模型加权聚合 (基于最终的“降权”名单) ---
                    weights = {cid: self.config["SUSPICIOUS_CLIENT_WEIGHT"] if cid in clients_to_downweight else 1.0
                ...
                # [Wandb] 2. 记录每一轮的指标
                # 我们将所有需要按轮次绘制的图表都记录在这里
                wandb.log({
                    "round": t,
                    "accuracy": accuracy,
                    "loss": test_loss,
                    "learning_rate": current_lr,
                    "active_clients": len(self.active_clients_pool)  # (可选) 追踪活跃客户端数量
                })
                print(
...
if __name__ == "__main__":
    ...
    if 1 in scenarios_to_run:
        ...
        # [Wandb] 1. 初始化一个新的运行
        run_name_1 = f"Scenario_1_No_Attack_Seed_{config['SEED']}"
        wandb.init(
            project="ic-ddw",
            name=run_name_1,
            config=config,
            reinit=True  # 允许在同一个脚本中多次初始化
        )
        ...
        # [Wandb] 2. 记录最终的总结性指标
        wandb.summary["final_accuracy"] = history_no_attack['accuracy'][-1] * 100
        wandb.summary["actual_rounds"] = actual_rounds_1
    ...
    if 2 in scenarios_to_run:
        ...
        # [Wandb] 1. 初始化一个新的运行
        run_name_2 = f"Scenario_2_Under_Attack_Seed_{config['SEED']}"
        wandb.init(
            project="ic-ddw",
            name=run_name_2,
            config=config,
        ...
        # [Wandb] 2. 记录最终的总结性指标
        wandb.summary["final_accuracy"] = history_under_attack['accuracy'][-1] * 100
        wandb.summary["actual_rounds"] = actual_rounds_2
    ...
    if 3 in scenarios_to_run:
        ...
        # [Wandb] 1. 初始化一个新的运行
        # 我们使用 f-string 来创建一个包含关键参数的、可读性强的运行名称
        run_name = f"T={config['OUTLIER_THRESHOLD']}_P={config['PERTURBATION_STRENGTH']}_Seed={config['SEED']}"
        wandb.init(
            project="ic-ddw",  # 项目名称，您可以自定义
            name=run_name,  # 本次运行的名称
            config=config, # 将我们的配置字典同步到Wandb，方便追溯
            reinit=True,
            mode = "offline"  # 开启离线模式
        )
    ...
    # [Wandb] 4. 记录最终的总结性指标
    final_accuracy = history_with_defense['accuracy'][-1] * 100
    malicious_removed = len(set(history_with_defense['removed_malicious']))
    benign_removed = len(set(history_with_defense['removed_benign']))
    wandb.summary["actual_rounds"] = len(history_with_defense['accuracy'])
    wandb.summary["final_accuracy"] = final_accuracy
    wandb.summary["malicious_removed"] = malicious_removed
    wandb.summary["benign_removed"] = benign_removed
