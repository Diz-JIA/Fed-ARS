...
@Time    : 2025/9/18
@Author  : dizjia
@Description: IDA+成对欧氏距离(Pairwise Euclidean Distance)
            从结合余弦相似度的实验结果ida_cos_simulation_log_scene3.csv来看，
            多数时候会误伤多个良性客户端
            遂测试一下PED效果如何
...
config = {
    ...
    # 实验日志文件路径
    "LOG_FILE_PATH": "./ida_ped_simulation_log_scene3.csv",
        ...
        try:
            for t in range(self.config["FL_ROUNDS"]):
                ...
                if self.config["DEFENSE_ENABLED"] and t + 1 >= self.config["DEFENSE_START_ROUND"]:
                    ...
                    # --- [新增] 2. 计算 Krum 离群分数 ---
                    # 一个客户端的最终“离群分数”，是它与其他所有客户端的成对欧氏距离中，最小的那 k 个距离的总和
                    krum_scores = {}
                    if len(client_updates) > 1:
                        # a. 将所有更新向量展平
                        client_ids_in_order = list(client_updates.keys())
                        all_updates_flat = torch.stack([
                            torch.cat([p.flatten() for p in client_updates[cid].values()])
                            for cid in client_ids_in_order
                        ])

                        # b. 计算两两之间的欧氏距离的平方 (计算平方和更高效)
                        # all_updates_flat 的形状是 (num_clients, num_params)
                        # dist_sq[i, j] 代表客户端i和j之间距离的平方
                        dist_sq = torch.cdist(all_updates_flat, all_updates_flat, p=2) ** 2

                        # c. 为每个客户端计算离群分数
                        # k 的经典取值是 n - f - 2，这里我们简化一下，取一个固定比例
                        # 例如，假设好人占一半以上，我们就取最近的 5 个邻居
                        num_neighbors = max(1, int(len(client_updates) * 0.5) - 1)

                        for i, client_id in enumerate(client_ids_in_order):
                            # 对第i行的所有距离进行排序
                            sorted_distances = torch.sort(dist_sq[i]).values
                            # 取从第2个到第k+1个的距离（第1个是自己到自己，距离为0）
                            # 我们取平方和，避免开根号，数值效果一样
                            score = torch.sum(sorted_distances[1:num_neighbors + 1])
                            krum_scores[client_id] = score.item()

                        # (可选) 打印Krum分数
                        # sorted_krum = sorted(krum_scores.items(), key=lambda item: item[1])
                        # print(f"    [调试信息] 本轮Krum离群分数(从低到高): {[(cid, round(val, 4))) for cid, val in sorted_krum]}")
                    ...
                    if len(delta_values) > 1:
                        ...
                        suspicious_ids = []
                        # 为Krum分数设定一个异常阈值，这需要通过统计方法确定
                        # 一个简单的方法是，Krum分数最高的20%被认为是异常的
                        if krum_scores:
                            krum_values = list(krum_scores.values())
                            # 例如，取第80百分位数作为阈值
                            # 第80百分位数就是那个比80 % 的客户端都高的分数，将这个分数设置为阈值
                            KRUM_THRESHOLD = np.percentile(krum_values, 80)
                        else:
                            KRUM_THRESHOLD = float('inf')

                        for client_id, delta in deltas.items():
                            score_mad = (delta - median) / mad
                            krum_score = krum_scores.get(client_id, 0)

                            # 混合判断：不稳定性异常 或 离群性异常
                            if score_mad > self.config["OUTLIER_THRESHOLD"] or krum_score > KRUM_THRESHOLD:
